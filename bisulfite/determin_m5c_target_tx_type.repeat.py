...
with open(args.input, 'r') as f:
    ...
    for line in f:
        ...
        if tx_type in tx_type_list:
            peakid_dict[peakid].append(tx_type)
        elif gene_type == 'intergenic':
            peakid_dict[peakid].append('Other repeat')
        else:
...
## exclude unknown from tx_type and re-calculate the total count
repeat_peak_count = 0
for tx_type in tx_type_list:
    if tx_type in feature_dict:
        repeat_peak_count += feature_dict[tx_type]

with open(args.output, 'w') as out:
    row = ['name', 'type', 'count', 'pct\n']
    out.write('\t'.join(row))
    for feature in tx_type_list:
        if feature in feature_dict:
            ## get count and pct
            count = feature_dict[feature]
            pct = count / repeat_peak_count
        else:
            count = 0
            pct = 0
