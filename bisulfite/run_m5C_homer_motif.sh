...
function Run_homer_motif {
  ...
  for bed in $BED_DIR/*.bed; do
    ...
    bsub -P ENCORI -q rhel8_standard -M 20000 -n 10 \
      -eo $LOG_DIR/$prefix.homer.err \
      -oo $LOG_DIR/$prefix.homer.log \
      -J homer_motif_${prefix} \
      "bash $SCRIPTS/sbatch_m5C_homer_motif.sh \
      $GENOME_FA \
      $GENOME_SIZE \
...
ANALYSIS_DIR="$BASE/analysis/motif_homer/caRNA/batch1"
LOG_DIR="$BASE/log/motif/caRNA/batch1"

rm -rf $LOG_DIR
mkdir -p $LOG_DIR

Run_homer_motif $BED_DIR $ANALYSIS_DIR
